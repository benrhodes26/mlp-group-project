...
class DataSet(object):
    #Input Data class    
    def __init__(self, data, max_time_steps, feature_len, n_distinct_questions, name=None):
        self.data = data
...
def GetDataSet(args):
    ...
    train = DataSet(train_set, max_time_steps, feature_len, n_distinct_questions, name="Train")
    valid = DataSet(val_set, max_time_steps, feature_len, n_distinct_questions, name="Valid")
    
    return train, valid
    

def run_epochs(sess, model, dataset, summary_merge, data_inputs, data_targets, data_target_ids, model_op=None):
...
0.1)
        ...
        with tf.name_scope("Train"):
            model_train = LstmModel(max_time_steps=train_set.max_time_steps, feature_len=train_set.feature_len,            
                                        n_distinct_questions=train_set.n_distinct_questions, is_training=True)

            with tf.variable_scope("Model", reuse=None, initializer=initializer):
                model_train.build_graph(n_hidden_units=200, learning_rate=args.learn_rate,
                                        decay_exp=args.decay, inputs=data_inputs, targets=data_targets, target_ids=data_target_ids)
        ...
        with tf.name_scope("Valid"):
            model_valid = LstmModel(max_time_steps=train_set.max_time_steps, feature_len=train_set.feature_len,            
                                        n_distinct_questions=train_set.n_distinct_questions, is_training=False)

            with tf.variable_scope("Model", reuse=True, initializer=initializer):
                model_valid.build_graph(n_hidden_units=200, inputs=data_inputs, targets=data_targets, target_ids=data_target_ids)
    ...
    with g.as_default():
        with tf.Session() as sess:
            ...
            for epoch in range(args.epochs):
                train_loss, train_accuracy, train_auc, train_summary = run_epochs(sess, model_train, train_set.data, merged,data_inputs, data_targets, data_target_ids, model_op=model_train.training)
                
                valid_loss, valid_accuracy, valid_auc, valid_summary = run_epochs(sess, model_train, valid_set.data, merged,data_inputs, data_targets, data_target_ids)
                train_writer.add_summary(train_summary)
                train_writer.add_summary(valid_summary)

                print("Epoch {}\nTrain:  Loss = {:.3f},  Accuracy = {:.3f},  AUC = {:.3f}\nValid:  Loss = {:.3f},  Accuracy = {:.3f},  AUC = {:.3f}"
                  .format(epoch, train_loss, train_accuracy, train_auc, valid_loss, valid_accuracy, valid_auc))
